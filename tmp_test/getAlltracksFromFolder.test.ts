...
describe('getAllTracksFromFolder', () => {
	...
	beforeAll(() => {
		...
		fs.writeFileSync(path.join(process.env.MUSIC_FOLDER!, guild_id, 'track1.mp3'), 'dummy content');
		fs.writeFileSync(path.join(process.env.MUSIC_FOLDER!, guild_id, 'track2.mp3'), 'dummy content');
		fs.writeFileSync(path.join(process.env.MUSIC_FOLDER!, guild_id, 'track3.mp3'), 'dummy content');
		fs.writeFileSync(path.join(process.env.MUSIC_FOLDER!, guild_id, 'not_a_track.txt'), 'dummy content');
	});
	...
	test('should throw ClientError with noTracksInServer message when folder is empty', () => {
		...
		fs.mkdirSync(path.join(process.env.MUSIC_FOLDER!, empty_guild_id), { recursive: true });
