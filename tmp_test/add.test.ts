...
describe('addCommand', () => {
	...
	test('Add track without guild folder and play false', async () => {
		fs.mkdirSync(process.env.MUSIC_FOLDER!, { recursive: true });
		...
		expect(fs.existsSync(path.join(process.env.MUSIC_FOLDER!, guild_id))).toBe(true);
		...
		expect(downloadTrack).toHaveBeenCalledWith(path.join(process.env.MUSIC_FOLDER!, guild_id), mockInteraction.options.getString('url'), mockInteraction.options.getAttachment('track'));
	...
	test('Add track without guild folder and play true', async () => {
		...
		expect(putTrackInPlayer).toHaveBeenCalledWith(
			mockInteraction,
			path.join(process.env.MUSIC_FOLDER!, guild_id),
			track_name,
			expect.any(Function),
		);
	});
	...
	test('Add track without guild folder and to_queue not set but not in voice channel', async () => {
		fs.mkdirSync(process.env.MUSIC_FOLDER!, { recursive: true });
		(downloadTrack as jest.Mock).mockResolvedValue(track_name);
		mockInteraction.options.getBoolean.mockReturnValue(null);
